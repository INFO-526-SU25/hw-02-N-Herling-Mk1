...
g3c <- ggplot(median_housing_q, aes(x = date, y = price)) +
  geom_line(color = "#4d72e3") +
  geom_point(shape = 1, size = 2, color = "#4d72e3") +  # Open circles
  
  scale_x_date(
    breaks = quarter_dates,
    labels = quarter_labels,
    limits = c(ymd("2019-01-01"), ymd("2020-10-01")) # - sets 'quarters' to graph
  ) +
  ...
  theme(
    ...
    axis.text.x = element_text(margin = margin(t = 10))  # space for sublabels
  ) +
  
  # - Add custom sublabels below the x-axis - set (x,y) coords of labels
  annotate("text", x = quarter_dates, y = -20000, label = year_labels, size = 3.5)
