...
median_housing <- read_csv("data/median-housing.csv") |>
  ...
  mutate(date = ymd(date))  # ✅ CHANGED: Replaced %>% with |>

# Load the recession data
recessions <- read_csv("data/recessions.csv") |>
  rename(start = Peak, end = Trough) |>
  mutate(start = ymd(start), end = ymd(end))  # ✅ CHANGED: Replaced %>% with |>
...
g1 <- ggplot(median_housing, aes(x = date, y = price)) +
  ...
  scale_x_date(
    breaks = x_breaks,
    limits = c(ymd("1960-01-01"), ymd("2025-01-01")),
    date_labels = "%Y"
  ) +
  ...
  scale_y_continuous(
    breaks = y_breaks, 
    labels = comma_format(),
    limits = c(0, 400000),     # ✅ FIXED: Lower limit now 40,000 instead of 0
    expand = c(0, 0)
  ) +
